...
select sum(total) as money from sale

-- 4. Получить уникальные store_id всех складов, с которых была хоть одна продажа
select distinct store_id from sale where sale_id
...
select name, avg(total/quantity) from product join sale using(product_id) where sale_id is not null group by name;
